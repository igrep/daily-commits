...
- wasm2arm32 compiler:
    - wasmのテストケースを実行できるARM32の実行ファイルを吐く
        - 2020/10/30:
            - Cで`1 << 32`したときとARM32で`LSL #1, #32`とで違う... Haskellの`1 `shiftL` 32 :: Word32`した結果はARM32と同じだった。私がまだちゃんとシフト演算を理解できてないってことか...
            - Ideoneでシフトした場合もまた違う。どうやらC言語の未定義動作らしい。だからWasmは未定義動作にしないために`i32.shl`は右辺を`mod 32`した上で計算しているのか。
- Haskell入門コンテンツ:
    - 課題9の不足箇所を埋める
        - `case`式の節を書き始め。
- [はじめてのOSコードリーディング --- UNIX V6で学ぶカーネルのしくみ](https://gihyo.jp/dp/ebook/2013/978-4-7741-5517-3)
...
- wasm2arm32 compiler:
    - wasmのテストケースを実行できるARM32の実行ファイルを吐く
        - `__divsi3`という関数が組み込みであることが分かったので呼んでみたら、あっさりテストが通った。`.global`とかで宣言する必要はなかったのかな
            - やっぱりgccの関数を呼ぶしかない？明日はまたobjdumpして見てみるか。
- Haskell-jp活動:
    - Monad則を破る話:
        - ReadmeTestの実装
            - 久々なので思い出すのに若干手こずったけど、テストのTODOをいくつか潰した。
            - `CompareAfterPrompt`のテストには、まだ最近の仕様変更に追いついていない箇所があるのでそこも書き直さないとね...
- [はじめてのOSコードリーディング --- UNIX V6で学ぶカーネルのしくみ](https://gihyo.jp/dp/ebook/2013/978-4-7741-5517-3)
- [圏論入門 Haskellで計算する具体例から](https://www.nippyo.co.jp/shop/book/8340.html)
- 中国語
...
- wasm2arm32 compiler:
    - wasmのテストケースを実行できるARM32の実行ファイルを吐く
        - 2020/10/26:
            - `sdiv`は使えないっぽい（オプションを指定すればいけそうだけど、ターゲットを広めにとっておきたい気はする）
            - で、gccが割り算をどういうアセンブリーにするかと思って探してみたけど結構難しいな。
        - 2020/10/27:
            - `__divsi3`という関数が組み込みであることが分かったので呼んでみたら、あっさりテストが通った。`.global`とかで宣言する必要はなかったのかな
- [はじめてのOSコードリーディング --- UNIX V6で学ぶカーネルのしくみ](https://gihyo.jp/dp/ebook/2013/978-4-7741-5517-3)
- [圏論入門 Haskellで計算する具体例から](https://www.nippyo.co.jp/shop/book/8340.html)
...
- wasm2arm32 compiler:
    - wasmのテストケースを実行できるARM32の実行ファイルを吐く
        ...
        - 2020/10/23: サクッと`i32.sub`、`i32.mul`も実装
- Haskell-jp活動:
    - 2020/10/21: 次回の会議のアジェンダ共有
    - 2020/10/22: COCのbranchをpublicにpushするところまで。緊張するから呼びかけは明日以降！
    - 2020/10/23: COCのPull requestを送った: <https://github.com/haskell-jp/community/pull/29>
    - Monad則を破る話:
        - ReadmeTestの実装
            ...
            - 2020/10/22 - 2020/10/23: 今日もちょっとだけ前進
- [はじめてのOSコードリーディング --- UNIX V6で学ぶカーネルのしくみ](https://gihyo.jp/dp/ebook/2013/978-4-7741-5517-3)
- [圏論入門 Haskellで計算する具体例から](https://www.nippyo.co.jp/shop/book/8340.html)
- 中国語
