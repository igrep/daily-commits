<!--
Execute this vim command to push updates of this file:

:w | !bash -c 'git add README.md ; git commit -m"update" ; git push'
-->

# daily-commits

GitHubのPublic Contribution Mapをどうしても途切れさせたくないために、毎日、OSSに何らかの形で寄与するか、何かしら技術を磨いたりブログ書いたりしたら更新する。
実

# 2017/08/30

- Haskellの記事: :star:
    - 編集して修正していただいた内容のレビュー
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:

# 2017/08/29

- Haskellコミュニティー活動 :star:
    - 昨日の発表を記事にしてた <https://github.com/haskell-jp/blog/pull/39>
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:

# 2017/08/28

- Haskellコミュニティー活動 :star:
    - [Haskell入門者LT会](https://shinjukuhs.connpass.com/event/58936/)にて発表。途中とんでもない言い間違いをしてしまったり、話しすぎて関心をそいでしまったようで、いろいろ失敗した。
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:

際にコミットした日と日記の日付が必ずしも一致しない点はご容赦を。

# 2017/08/27

- Haskellコミュニティー活動 :star:
    - 明日[Haskell入門者LT会](https://shinjukuhs.connpass.com/event/58936/)に向けて準備
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:

# 2017/08/26

- タイプセーフプリキュア: :star:
    - プチプリキュアハッカソンにて時間オーバーしつつも、ようやくGHCのANNを使ったバージョンでプリアラのJSONを構築することができた。

# 2017/08/25

- タイプセーフプリキュア: :star:
    - ううっ、型が合わない。。。

# 2017/08/23 - 2017/08/24

- タイプセーフプリキュア: :star:
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
    - 読了。

# 2017/08/21 - 2017/08/22

- タイプセーフプリキュア: :star:
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/08/20

- タイプセーフプリキュア: :star:
    - 昨日考えた方針を見直した。

# 2017/08/19

- Haskellの記事: :star:
    - 一旦脱稿。あとは編集さんのアドバイスを聞きながら調整
- タイプセーフプリキュア: :star:
    - キュアインデックスに向けて開発開始。大体の設計方針を決め、最小限の実装を試してみた。

# 2017/08/07 - 2017/08/18

- Haskellの記事: :star:
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/08/06

- Haskellの記事: :star:
    - ようやく苦戦していた節を書き切る。ソシャゲも適度にやりながら書けるようになってきた。もちろん、それだけやりこんで夢中になる要素がなくなってしまった、ということでもあるけど...。
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- Haskellコミュニティー活動 :star:
    - takenobuさんの書いていただいた記事のレビュー
        - <https://github.com/haskell-jp/blog/pull/26> 
        - <https://github.com/haskell-jp/blog/pull/27>

# 2017/08/05

- Haskellの記事: :star:
    - スランプ気味。何を書いても理解できるものになっているのか、自信が持てない。それでついソシャゲに逃げてしまう。
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/07/31 - 2017/08/04

- Haskellの記事: :star:
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/07/30

- Haskellの記事: :star: :star:
    - Haskellもくもく会のおかげで思ったよりはかどった
- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/07/29

- Haskellの記事: :star:
    - 障害対応の疲れからか、今日もろくに何もせず。

# 2017/07/28

- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/07/27

- [Notes on category theory in the context of (functional) programming](https://github.com/jwbuurlage/category-theory-programmers): :star:

# 2017/07/26

- Haskellの記事: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/07/25

- Haskellの記事: :star:
    - いつにもなくDMMのソーシャルゲームに夜中中夢中になってしまったので本は読まず。

# 2017/07/24

- Haskellの記事: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/23

- Haskellの記事: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:
- Haskellコミュニティー活動 :star:
    - [GHC 8.2が出たのでリリースノートを抄訳](https://www.reddit.com/r/haskell_jp/comments/6ozjh0/announce_ghc_821_available/)

# 2017/07/20 - 2017/07/22

- Haskellの記事: :star:
    - ようやく最終的な構成が見えてきた。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/16 - 2017/07/19

- Haskellの記事: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/15

- Haskellの記事: :star:
    - [Haskell入門ハンズオン](https://shinjukuhs.connpass.com/event/58224/)にてメンターをやっていたためほとんど各時間はとれなかったが、少しでも進められた。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:
- Haskellコミュニティー活動 :star:
    - [Haskell入門ハンズオン](https://shinjukuhs.connpass.com/event/58224/)にてメンター。いろいろ褒めてもらえてうれしかったのでメモ。人によっていろいろ視点が違って、思わぬ質問が出るのも印象的だった。難しいね。やっぱり人に教えるのは。

# 2017/07/13 - 2017/07/14

- Haskellの記事: :star:
    - やや順調？
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/12

- Haskellの記事: :star:
    - ようやく一段落？
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/05 - 2017/07/11

- Haskellの記事: :star:
    - またノってきた？
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/04

- Haskellの記事: :star:
- [haddockのバグ修正](https://github.com/haskell/haddock/pull/566):
    - ようやくマージしてもらえた。いろいろ助けてもらいながらだったのでありがたい。しかしunstableなソフトウェアをいじるのは大変ね...。はていくつのバグに悩まされたか...。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/07/01 - 2017/07/03

- Haskellの記事: :star:
    - うーん、ちょっとずつ進んではいるものの、どうも楽しんで書けていない...。こんなペースで大丈夫なのかもわからないし...。
- [haddockのバグ修正](https://github.com/haskell/haddock/pull/566):
    - 金曜日にようやく解決策らしきものが見つかったので試すも、haddockのmasterがすでにGHC 8.2向けのものになっており、手元のWindowsにGHC 8.2を入れてみるも、GHC 8.2でcabal-installがうまくがビルドできず、テストできない...。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/27 - 2017/06/30

- Haskellの記事: :star:
    - あと2日でどこまでを目標にする？
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/26

- Haskellの記事: :star:
    - vimがトラブり、あまり進まず。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/25

- Haskellの記事: :star:
    - ブラックジャックアプリケーション作りの事例を説明し続けるのに限界を感じてきたので、一旦戻って開発環境の構築から始めることに。案の定簡単にかける箇所なのでサクサクと進んだ。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/24

- Haskellの記事: :star:
    - なかなか進まず...。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/19 - 2017/06/23

- Haskellの記事: :star:
    - 大体の締め切りを早めに見積もって残り9日しかないことがわかって焦る。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/18

- Haskellコミュニティー活動: :star:
    - redditについて発表
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/12 - 2017/06/17

- Haskellの記事: :star:
    - 少しずつ筆が乗ってきていい感じ？
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/11

- PowerShellでWindowsのセットアップスクリプト作り: :star:
    - sshでWindowsにログインしてみて気づいた点を中心に、環境変数の設定方法を修正したり。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:
- Haskell-jpコミュニティー活動: :star:
    - ふっと思いつきで新しいロゴを作っていた。以前から考えていたアイディアを試せたのでよしよし。
- やっぱり睡眠不足になっていたのか、Haskellの記事にはほぼ手をつけられず。

# 2017/06/10

- PowerShellでWindowsのセットアップスクリプト作り: :star:
    - sshでWindowsにログインしてみて気づいた点を中心に、環境変数の設定方法を修正したり。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/06 - 2017/06/09

- Haskellの記事: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/06/05

- タイプセーフプリキュア: :star:
    - キラキラ☆プリキュアアラモードに新たに追加された技追加

# 2017/06/02 - 2017/06/04

- Haskellの記事: :star:
    - サンプル用のコードを書いた。

# 2017/06/01

- 作りながら学ぶHaskell入門: :star:
- Haskellの記事: :star:
    - 題材を相談。多分決定。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [アジャイルレトロスペクティブズ 強いチームを育てる「ふりかえり」の手引き](https://estore.ohmsha.co.jp/titles/978427406698P): :star:

# 2017/05/31

- 作りながら学ぶHaskell入門: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:

# 2017/05/30

- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:
    - 読了。

# 2017/05/29

- 作りながら学ぶHaskell入門: :star:
- Haskellの記事: :star:
   - おおまかな方針を相談
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/28

- 作りながら学ぶHaskell入門: :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/22 - 2017/05/27

- 作りながら学ぶHaskell入門: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/21

- 作りながら学ぶHaskell入門: :star:
- Haskell-jpコミュニティー活動: :star:
    - 細々した雑務。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/19 - 2017/05/20

- 作りながら学ぶHaskell入門: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/18

- 作りながら学ぶHaskell入門: :star:
- Haskell-jpコミュニティー活動: :star:
    - [相互リンク集のPR](https://github.com/haskell-jp/blog/pull/20)をマージ

# 2017/05/15 - 2017/05/17

- 作りながら学ぶHaskell入門: :star:
- Haskell-jpコミュニティー活動: :star:
    - 引き続き相互リンク集追加希望者探し。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/14

- Haskell-jpコミュニティー活動: :star:
    - 相互リンク集追加希望者探し。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/12 - 2017/05/13

- 作りながら学ぶHaskell入門: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/11

- 作りながら学ぶHaskell入門: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:
- 筋トレ: :muscle:
- 股割: :muscle:

# 2017/05/09 - 2017/05/10

- keep-me-contributing for Android: :star:
    - どうもIntelliJの調子が悪く（メモリーやネットワークの速度が足りてない？）、ほとんど進まない。。。
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:
- 筋トレ: :muscle:
- 股割: :muscle:

# 2017/05/08

- keep-me-contributing for Android: :star:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/07

- keep-me-contributing for Android: :star:
    - Android Widgetの作り方について調べてた
- 筋トレ: :muscle:
- 股割: :muscle:

# 2017/05/06

- Haskell-jpコミュニティー活動: :star:
    - [typo修正](https://github.com/haskell-jp/blog/pull/21)
- Haskell本のレビュー: :blue_book:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/05

- Haskell-jpコミュニティー活動: :star:
    - [相互リンク集のPull Request](https://github.com/haskell-jp/blog/pull/20)を出した。今のところ追加希望者が一人もいなくて悲しい :cry:
- 筋トレ: :muscle:
- 股割: :muscle:

# 2017/05/03 - 2017/05/04

- Haskell-jpコミュニティー活動: :star:
    - 相互リンク集の作成
- Haskell本のレビュー: :blue_book:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:

# 2017/05/01 - 2017/05/02

- Haskell-jpコミュニティー活動: :star:
    - ロゴを少し修正したり、第1回Haskell-jpもくもく会のイベントページを作ったり
- 作りながら学ぶHaskell入門: :star:
- Haskell本のレビュー: :blue_book:
- [アート・オブ・コミュニティ](https://www.oreilly.co.jp/books/9784873114958/): :star:
- [スクラム現場ガイド](https://book.mynavi.jp/ec/products/detail/id=50668): :star:
